...
async def messagereply(message, messagestring):
    ...
    if 'creeper' in messagestring and replies == 1:
        ...
        if sounds == 1:
            await checksoundreaction('focus', messagestring, message, config_file.focus_mp3_path)
        if replies == 1:
            await checkwordreaction('focus', 'None', messagestring, message, config_file.focus_mp4_path, 'None')
    elif 'pizza' in messagestring:
        if sounds == 1:
            await checksoundreaction('pizza', messagestring, message, config_file.focus_mp3_path)
        if replies == 1:
            await checkwordreaction('pizza', 'None', messagestring, message, config_file.focus_mp4_path, 'None')
